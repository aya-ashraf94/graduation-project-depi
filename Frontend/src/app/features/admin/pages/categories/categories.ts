...
interface AttrForm { name: string; type: string; options: string; required: boolean; hasOther: boolean; }

@Component({
  selector: 'app-admin-categories',
  standalone: true,
  imports: [CommonModule, FormsModule, ReactiveFormsModule, FormFieldComponent, AdminErrorPanelComponent, AdminLoaderComponent],
...
export class AdminCategories implements OnInit {
  ...
  openEditModal(cat: any): void {
    ...
    this.formAttributes = (cat.attributes || []).map((a: any) => ({
      name: a.name, type: a.type,
      options: Array.isArray(a.options) ? a.options.join(', ') : '',
      required: a.required, hasOther: a.hasOther,
    }));
    this.showModal.set(true);
  }
  ...
  saveCategory(): void {
    ...
    if (!name) { this.toastService.error('Category name is required.'); return; }
    ...
    obs.subscribe({
      next: () => {
        ...
        this.saving.set(false); this.closeModal(); this.loadCategories();
      },
      error: (err) => { this.toastService.error(err?.error?.message || 'Failed to save category.'); this.saving.set(false); }
    });
  }
  ...
  deleteCategory(id: string, name: string): void {
    this.confirmService.show({
      title: 'Delete Category', message: `Delete "${name}" and all its attributes? This cannot be undone.`,
      onConfirm: () => {
        this.http.delete(`${this.apiUrl}/${id}`).subscribe({
          next: () => { this.toastService.success(`Category "${name}" deleted.`); this.loadCategories(); },
  ...
  @HostListener('document:keydown.escape') onEscape(): void { this.showModal.set(false); }
}
